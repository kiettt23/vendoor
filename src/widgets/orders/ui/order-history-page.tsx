            ...
            <Link key={order.id} href={`/orders/${order.id}`}>
              <Card className="hover:shadow-md transition-shadow cursor-pointer">
                <CardHeader className="pb-2">
                  ...
                  <p className="text-sm text-muted-foreground">{order.vendor.shopName}</p>
                </CardHeader>
                <CardContent>
                  <div className="flex items-center justify-between">
                    <div>
                      <p className="text-sm">{order.items.map((i) => `${i.productName} x${i.quantity}`).join(", ")}
                        {order._count.items > 2 && ` +${order._count.items - 2} sản phẩm`}</p>
