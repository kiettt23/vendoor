...
import { ArrowLeft, Package, User, MapPin, Store, CreditCard } from "lucide-react";
...
export async function AdminOrderDetailPage({ orderId }: AdminOrderDetailPageProps) {
  ...
  if (!order) {
    return (
      <div className="container mx-auto py-16 px-4 text-center">
        ...
        <Button asChild><Link href="/admin/orders">Về danh sách đơn hàng</Link></Button>
      </div>
    );
  }
  ...
  return (
    <div className="container mx-auto py-8 px-4 max-w-4xl">
      <Button variant="ghost" size="sm" asChild className="mb-6">
        <Link href="/admin/orders"><ArrowLeft className="mr-2 h-4 w-4" />Danh sách đơn hàng</Link>
      </Button>
      ...
      <div className="grid md:grid-cols-3 gap-6 mb-8">
        <Card>
          <CardHeader className="pb-3"><CardTitle className="text-base flex items-center gap-2"><User className="h-4 w-4" />Khách hàng</CardTitle></CardHeader>
          <CardContent>
        ...
        <Card>
          <CardHeader className="pb-3"><CardTitle className="text-base flex items-center gap-2"><Store className="h-4 w-4" />Nhà bán</CardTitle></CardHeader>
          <CardContent>
            <p className="font-medium">{order.vendor.shopName}</p>
          </CardContent>
        </Card>
        <Card>
          <CardHeader className="pb-3"><CardTitle className="text-base flex items-center gap-2"><MapPin className="h-4 w-4" />Giao hàng</CardTitle></CardHeader>
          <CardContent>
      ...
      <Card className="mb-8">
        ...
        <CardContent className="space-y-4">
          ...
          <div className="space-y-2">
            ...
            <div className="flex justify-between text-blue-600"><span>Phí nền tảng ({(order.platformFeeRate * 100).toFixed(0)}%)</span><span>{formatPrice(order.platformFee)}</span></div>
            <div className="flex justify-between text-green-600"><span>Vendor thu</span><span>{formatPrice(order.vendorEarnings)}</span></div>
            <Separator />
            <div className="flex justify-between font-bold text-lg"><span>Tổng đơn hàng</span><span className="text-primary">{formatPrice(order.total)}</span></div>
          </div>
        </CardContent>
      </Card>
