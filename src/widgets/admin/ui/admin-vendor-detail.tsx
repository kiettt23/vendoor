...
export async function AdminVendorDetailPage({
  vendorId,
}: AdminVendorDetailPageProps) {
  const vendor = await getVendorDetailForAdmin(vendorId);
    ...
    <div className="container mx-auto py-8 px-4 max-w-4xl">
      ...
      <div className="grid md:grid-cols-2 gap-6 mb-8">
        <Card>
          ...
          <CardContent>
            <p className="font-semibold">{vendor.user.name || "N/A"}</p>
            <p className="text-sm text-muted-foreground">{vendor.user.email}</p>
            <p className="text-sm text-muted-foreground">
              {vendor.user.phone || "Chưa có SĐT"}
            </p>
            <p className="text-sm text-muted-foreground mt-2">
              Đăng ký: {formatDate(vendor.user.createdAt)}
            </p>
          </CardContent>
        </Card>
        <Card>
          ...
          <CardContent className="space-y-2">
            <div className="flex justify-between">
              ...
              <span className="font-medium">{vendor.stats.totalProducts}</span>
            </div>
            <div className="flex justify-between">
              <span className="flex items-center gap-2">
            ...
            <div className="flex justify-between">
              ...
              <span className="font-bold text-primary">
                {formatPrice(vendor.stats.totalRevenue)}
              </span>
            </div>
          </CardContent>
        </Card>
      ...
      {vendor.description && (
        <Card className="mb-8">
          <CardHeader>
            <CardTitle>Mô tả shop</CardTitle>
          </CardHeader>
          <CardContent>
        ...
        <CardContent>
          <div className="flex gap-2 flex-wrap">
            {vendor.status === "PENDING" && (
              <>
                <form
                  action={updateVendorStatus.bind(null, vendor.id, "APPROVED")}
                >
                  <Button type="submit">
                ...
                <form
                  action={updateVendorStatus.bind(null, vendor.id, "REJECTED")}
                >
            ...
            {vendor.status === "APPROVED" && (
              <form
                action={updateVendorStatus.bind(null, vendor.id, "SUSPENDED")}
              >
